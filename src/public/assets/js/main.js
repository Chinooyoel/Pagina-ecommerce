...
function funcionBuscarProductoParaTabla() {
	...
	buscadorTabla.addEventListener(
		...
		e => {
			...
			const valorDelBuscador = obtenerCampoDeFormulario(buscadorTabla, 'nombre')
...
function mostrarProductoEnTabla(productoArray) {
	...
	if (productoArray.length === 0) {
		return (tablaProducto.innerHTML +=
			'<th colspan="7">No hay resultados de la busqueda</th>')
	}
...
function funcionBuscadorGeneral() {
	const buscadoresGenerales = document.getElementsByClassName('buscadorGeneral')
	let valorBuscador
...
function mostrarProductosDelCarrito() {
	...
	if (productos.length === 0) {
		subtitular.innerHTML =
			'<i class="fa fa-times-circle icono"></i>No hay producto en el carrito'
		return
	}
...
function funcionBuscarPedidoParaTabla() {
	...
	buscadorTabla.addEventListener(
		...
		e => {
			...
			//obtenemos el valor del campo
			const valorDelBuscador = obtenerCampoDeFormulario(buscadorTabla, 'codigo')
			const filtroEstado = obtenerCampoDeFormulario(buscadorTabla, 'estado')
