...
function funcionBuscarProductoParaTabla () {
  const buscadorTabla = document.getElementById('buscarProducto');
...
function funcionMostrarFoto () {
  const botonInputFileEstilizado = document.getElementById(
    'botonInputFileEstilizado'
  );
  const inputFile = document.getElementById('elegirFoto');
  const output = document.getElementById('imagenProducto');
...
function copiarEnlace () {
  const botonCopiarEnlace = document.getElementById('botonCopiarEnlace');
  const modalCopiarEnlace = document.getElementById('modalCopiarEnlace');
...
function ordenarProductos () {
  const opcionOrdenar = document.getElementById('ordenar');

  opcionOrdenar.addEventListener('input', (e) => {
    if (e.target.value != '') { window.location.href = `${window.location.origin}${e.target.value}`; }
  });
}
...
function actualizarRolUsuario () {
  ...
  formUsuarioRol.addEventListener('submit', async e => {
    ...
    // hacemos la peticion
    const resultado = await fetch(miPeticion);
...
function enviarPeticion (metodo, url, callback) {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      const respuestaObj = JSON.parse(this.responseText);
...
function cerrarSesion () {
  const botonCerrarSesion = document.getElementsByClassName('botonCerrarSesion');
...
function leerCarrito () {
  if (!localStorage.carrito) {
    ...
    const carritoObj = JSON.parse(localStorage.carrito);
    return new Carrito(
...
function mostrarProductosDelCarrito () {
  ...
  const interfazComprar = document.getElementById('interfazComprar');
...
function eventoEnviarCarrito () {
  ...
  carritoForm.addEventListener('submit', async (e) => {
    ...
    const objetoFormulario = new FormData(carritoForm);
    ...
    const miPeticion = new Request('/pedido/crear', {
      method: 'POST',
...
function funcionBuscarPedidoParaTabla () {
  const buscadorTabla = document.getElementById('buscarPedido');
...
function leerCookie (nombre) {
  ...
  for (let i = 0; i < ca.length; i++) {
    ...
    if (c.indexOf(nombreIgual) == 0) { return c.substring(nombreIgual.length, c.length); }
  }
  return null;
}
