...
function elegirMarca () {
  const arrayMarca = document.getElementsByClassName('marcaLogo');
...
function cambiarPasoYTotal () {
  ...
  const pasoHTML = document.getElementsByClassName('paso')[paso];
  const cajaTexto = pasoHTML.getElementsByClassName('textoInstructivo')[0];
  ...
  // cambiamos total
  const total = calcularPrecioTotalDeComponentes();
...
async function consultarProductos () {
  const resultado = await fetch(`/producto/buscar/palabra=${componentesPc[paso].palabra}/idcat=${componentesPc[paso].categoria}/idsub=${componentesPc[paso].subcategoria}/idmarca=${componentesPc[paso].marca}/orden=ASC?JSON=true`);
...
function cargarVistaComponentesAElegir (productosDB) {
  const listaProducto = document.getElementsByClassName('listaProducto')[0];
...
function agregarComponente () {
  ...
  for (let i = 0; i < botonesSeleccionarComponente.length; i++) {
    botonesSeleccionarComponente[i].addEventListener('click', (e) => {
      const elementoPadre = e.target.parentNode.parentNode;
...
function modificarComponente () {
  ...
  for (let i = 0; i < botonesModificarComponentes.length; i++) {
    botonesModificarComponentes[i].addEventListener('click', async (e) => {
      const pasoAModificar = e.target.dataset.paso;
