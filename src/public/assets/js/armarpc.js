...
async function consultarProductos() {
	let resultado = await fetch(
		`/producto/buscar/palabra=${componentesPc[paso].palabra}/idcat=${componentesPc[paso].categoria}/idsub=${componentesPc[paso].subcategoria}/idmarca=${componentesPc[paso].marca}/orden=ASC?JSON=true`,
	)
...
function agregarComponente() {
	const botonesSeleccionarComponente = document.getElementsByClassName(
...
function modificarComponente() {
	const botonesModificarComponentes = document.getElementsByClassName(
		'modificarComponente',
	)
...
function agregarComponentesAlCarrito() {
	for (let i = 0; i < componentesPc.length; i++) {
		...
		if (
			componentesPc[i].producto.id === -1 ||
			componentesPc[i].producto === ''
		) {
			continue
		}
