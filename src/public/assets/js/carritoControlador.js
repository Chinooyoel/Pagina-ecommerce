class Carrito {
	...
	leerProducto(productoDOM) {
		const id = Number(
			productoDOM.querySelector('[name=nombreProducto]').dataset.id,
		)
		const nombre = productoDOM.querySelector(
			'[name=nombreProducto]',
		).textContent
		const precio = Number(
			productoDOM.querySelector('[name=precioProducto]').textContent,
		)
		let img
	...
	eliminarProducto(producto) {
		...
		if (posicion != -1) {
			...
			this.total -=
				this.productos[posicion].cantidad * this.productos[posicion].precio

			this.productos = this.productos.filter(
				elemento => elemento.id != this.productos[posicion].id,
			)
		}
	}
}
